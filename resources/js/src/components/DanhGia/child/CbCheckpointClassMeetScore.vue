...
export default {
    ...
    methods:{
        ...
        async increment(userIndex, scoreIndex, step) {
            let scores = this.userScoreListClone[userIndex].scoreList;
            let originScore = scores[scoreIndex];
            originScore.cbl_score = Number(originScore.cbl_score) == Number(originScore.max_score) ?
                Number(originScore.cbl_score) : +originScore.cbl_score + step;
            await this.updateValue(userIndex, scoreIndex, originScore.cbl_score)
        },
        async decrement(userIndex, scoreIndex, step) {
            let scores = this.userScoreListClone[userIndex].scoreList;
            let originScore = scores[scoreIndex];
            originScore.cbl_score = originScore.cbl_score == 0 ? 0 : +originScore.cbl_score - step;
            await this.updateValue(userIndex, scoreIndex, originScore.cbl_score);
        },
        generateUid() {
